...
Trying first with a Seurat object - warning, these are all quite very large...
...
A possibility: drop the HTOsPool1-6?

```{r}
pryr::object_size(seurat_c1_full_FG)
seurat_c1_full_FG[["HTOsPool1"]] <- NULL
seurat_c1_full_FG[["HTOsPool2"]] <- NULL
seurat_c1_full_FG[["HTOsPool3"]] <- NULL
seurat_c1_full_FG[["HTOsPool4"]] <- NULL
seurat_c1_full_FG[["HTOsPool5"]] <- NULL
seurat_c1_full_FG[["HTOsPool6"]] <- NULL
pryr::object_size(seurat_c1_full_FG)

# Ok, not too much difference...
```
...
sce_pbmcs_rhapsody <-  readH5AD("pbmc_rhapsody_processed_cellXgene.h5ad")
...
pryr::object_size(sce_rt)
pryr::object_size(sce_neu_rhapsody)
pryr::object_size(sce_pbmcs_10x)
pryr::object_size(sce_pbmcs_rhapsody)
...
which leads to a call with...

```{r}
# if one wants to try out the voice command features...
iSEE(sce_rt, initial = initial, voice = TRUE)
```
...
Again, doing something live, and then exporting the initial config
...
```{r}
iSEE(sce_neu_rhapsody, initial = initial)
```
...
Let's have a look at a larger one.

This is included in fig 2D

```{r}
sce_pbmcs_rhapsody
iSEE(sce_pbmcs_rhapsody)

# to facilitate the buildup:
library(iSEEu)
modeEmpty(sce_pbmcs_rhapsody)
```

We explore live, export the config...
...
initial[["ReducedDimensionPlot1"]] <- new("ReducedDimensionPlot", Type = "X_umap", XAxis = 1L, YAxis = 2L, 
    FacetRowByColData = "orig.ident", FacetColumnByColData = "orig.ident", 
    ColorByColumnData = "cluster_labels_res.0.4", ColorByFeatureNameAssay = "X", 
    ColorBySampleNameColor = "#FF0000", ShapeByColumnData = "orig.ident", 
    SizeByColumnData = "nCount_RNA", FacetRowBy = "None", FacetColumnBy = "None", 
    ColorBy = "Column data", ColorByDefaultColor = "#000000", 
    ColorByFeatureName = "MT-CO1", ColorByFeatureSource = "---", 
    ColorByFeatureDynamicSource = FALSE, ColorBySampleName = "0", 
    ColorBySampleSource = "---", ColorBySampleDynamicSource = FALSE, 
    ShapeBy = "None", SizeBy = "None", SelectionAlpha = 0.1, 
    ZoomData = numeric(0), BrushData = list(), VisualBoxOpen = TRUE, 
    VisualChoices = c("Color", "Point"), ContourAdd = FALSE, 
    ContourColor = "#0000FF", PointSize = 1, PointAlpha = 1, 
    Downsample = TRUE, DownsampleResolution = 150, CustomLabels = FALSE, 
    CustomLabelsText = "0", FontSize = 1, LegendPointSize = 1, 
    LegendPosition = "Bottom", HoverInfo = TRUE, LabelCenters = FALSE, 
    LabelCentersBy = "orig.ident", LabelCentersColor = "#000000", 
    VersionInfo = list(iSEE = structure(list(c(2L, 4L, 0L)), class = c("package_version", 
...
initial[["RowDataTable1"]] <- new("RowDataTable", Selected = "IFIT3", Search = "IFIT", SearchColumns = "", 
    HiddenColumns = character(0), VersionInfo = list(iSEE = structure(list(
        c(2L, 4L, 0L)), class = c("package_version", "numeric_version"
...
initial[["FeatureAssayPlot1"]] <- new("FeatureAssayPlot", Assay = "X", XAxis = "Column data", XAxisColumnData = "cluster_labels_res.0.4", 
    XAxisFeatureName = "MT-CO1", XAxisFeatureSource = "---", 
    XAxisFeatureDynamicSource = FALSE, YAxisFeatureName = "IFIT3", 
    YAxisFeatureSource = "RowDataTable1", YAxisFeatureDynamicSource = FALSE, 
    FacetRowByColData = "orig.ident", FacetColumnByColData = "orig.ident", 
    ColorByColumnData = "orig.ident", ColorByFeatureNameAssay = "X", 
    ColorBySampleNameColor = "#FF0000", ShapeByColumnData = "orig.ident", 
    SizeByColumnData = "nCount_RNA", FacetRowBy = "None", FacetColumnBy = "None", 
    ColorBy = "None", ColorByDefaultColor = "#000000", ColorByFeatureName = "MT-CO1", 
    ColorByFeatureSource = "---", ColorByFeatureDynamicSource = FALSE, 
    ColorBySampleName = "0", ColorBySampleSource = "---", ColorBySampleDynamicSource = FALSE, 
    ShapeBy = "None", SizeBy = "None", SelectionAlpha = 0.1, 
...
initial[["ReducedDimensionPlot2"]] <- new("ReducedDimensionPlot", Type = "X_umap", XAxis = 1L, YAxis = 2L, 
    ...
    SizeByColumnData = "nCount_RNA", FacetRowBy = "None", FacetColumnBy = "None", 
    ColorBy = "Feature name", ColorByDefaultColor = "#000000", 
    ColorByFeatureName = "IFIT3", ColorByFeatureSource = "RowDataTable1", 
    ColorByFeatureDynamicSource = FALSE, ColorBySampleName = "0", 
    ColorBySampleSource = "---", ColorBySampleDynamicSource = FALSE, 
    ShapeBy = "None", SizeBy = "None", SelectionAlpha = 0.1, 
    ZoomData = numeric(0), BrushData = list(), VisualBoxOpen = TRUE, 
...
initial[["ReducedDimensionPlot3"]] <- new("ReducedDimensionPlot", Type = "X_umap", XAxis = 1L, YAxis = 2L, 
    FacetRowByColData = "orig.ident", FacetColumnByColData = "orig.ident", 
    ColorByColumnData = "disease_stage", ColorByFeatureNameAssay = "X", 
    ColorBySampleNameColor = "#FF0000", ShapeByColumnData = "orig.ident", 
    SizeByColumnData = "nCount_RNA", FacetRowBy = "None", FacetColumnBy = "None", 
    ColorBy = "Column data", ColorByDefaultColor = "#000000", 
    ColorByFeatureName = "MT-CO1", ColorByFeatureSource = "---", 
...
initial[["ComplexHeatmapPlot1"]] <- new("ComplexHeatmapPlot", Assay = "X", CustomRows = TRUE, CustomRowsText = "LYZ\nTYROBP\nCSTA\nKLF4\nCLEC7A\nMARCKS\nIFITM3\nISG15\nDEFA3\nDEFA4", 
    ClusterRows = FALSE, ClusterRowsDistance = "spearman", ClusterRowsMethod = "ward.D2", 
    DataBoxOpen = FALSE, VisualChoices = "Annotations", ColumnData = "cluster_labels_res.0.4", 
    RowData = character(0), CustomBounds = FALSE, LowerBound = NA_real_, 
    UpperBound = NA_real_, AssayCenterRows = FALSE, AssayScaleRows = FALSE, 
    DivergentColormap = "purple < black < yellow", ShowDimNames = "Rows", 
    LegendPosition = "Bottom", LegendDirection = "Horizontal", 
    ...
    ShowColumnSelection = TRUE, OrderColumnSelection = TRUE, 
    VersionInfo = list(iSEE = structure(list(c(2L, 4L, 0L)), class = c("package_version", 
    "numeric_version"))), PanelId = 1L, PanelHeight = 500L, PanelWidth = 8L, 
    SelectionBoxOpen = FALSE, RowSelectionSource = "---", ColumnSelectionSource = "---", 
    RowSelectionDynamicSource = FALSE, ColumnSelectionDynamicSource = FALSE, 
    RowSelectionRestrict = FALSE, ColumnSelectionRestrict = FALSE, 
    SelectionHistory = list())
...
and then call iSEE again with that setup:

```{r}
iSEE(sce_pbmcs_rhapsody, initial = initial)
```

A possible idea is to remove the heatmap to speed it up - coupled with grid downsampling of the points, it would help nicely
